...
export class AuthService {
  ...
  // apiurl = 'https://devs-freelance.herokuapp.com/';

  user = new BehaviorSubject<User>(null);
  constructor(private http: HttpClient, private router: Router) {}

  signup(account: signupModel) {
    return this.http.post<AuthResData>(this.apiurl + 'signup/', account).pipe(
  ...
  login(account: loginModel) {
    return this.http.post<AuthResData>(this.apiurl + 'login/', account).pipe(
      catchError(this.handleError),
      tap((res) => {
        this.handleAuth(res);
      })
